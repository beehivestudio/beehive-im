/* Generated by the protocol buffer compiler.  DO NOT EDIT! */
/* Generated from: mesg.proto */

/* Do not generate deprecated warnings for self */
#ifndef PROTOBUF_C__NO_DEPRECATED
#define PROTOBUF_C__NO_DEPRECATED
#endif

#include "mesg.pb-c.h"
void   mesg_online_req__init
                     (MesgOnlineReq         *message)
{
  static MesgOnlineReq init_value = MESG_ONLINE_REQ__INIT;
  *message = init_value;
}
size_t mesg_online_req__get_packed_size
                     (const MesgOnlineReq *message)
{
  assert(message->base.descriptor == &mesg_online_req__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_online_req__pack
                     (const MesgOnlineReq *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_online_req__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_online_req__pack_to_buffer
                     (const MesgOnlineReq *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_online_req__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgOnlineReq *
       mesg_online_req__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgOnlineReq *)
     protobuf_c_message_unpack (&mesg_online_req__descriptor,
                                allocator, len, data);
}
void   mesg_online_req__free_unpacked
                     (MesgOnlineReq *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_online_req__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_online_ack__init
                     (MesgOnlineAck         *message)
{
  static MesgOnlineAck init_value = MESG_ONLINE_ACK__INIT;
  *message = init_value;
}
size_t mesg_online_ack__get_packed_size
                     (const MesgOnlineAck *message)
{
  assert(message->base.descriptor == &mesg_online_ack__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_online_ack__pack
                     (const MesgOnlineAck *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_online_ack__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_online_ack__pack_to_buffer
                     (const MesgOnlineAck *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_online_ack__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgOnlineAck *
       mesg_online_ack__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgOnlineAck *)
     protobuf_c_message_unpack (&mesg_online_ack__descriptor,
                                allocator, len, data);
}
void   mesg_online_ack__free_unpacked
                     (MesgOnlineAck *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_online_ack__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_join_req__init
                     (MesgJoinReq         *message)
{
  static MesgJoinReq init_value = MESG_JOIN_REQ__INIT;
  *message = init_value;
}
size_t mesg_join_req__get_packed_size
                     (const MesgJoinReq *message)
{
  assert(message->base.descriptor == &mesg_join_req__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_join_req__pack
                     (const MesgJoinReq *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_join_req__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_join_req__pack_to_buffer
                     (const MesgJoinReq *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_join_req__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgJoinReq *
       mesg_join_req__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgJoinReq *)
     protobuf_c_message_unpack (&mesg_join_req__descriptor,
                                allocator, len, data);
}
void   mesg_join_req__free_unpacked
                     (MesgJoinReq *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_join_req__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_join_ack__init
                     (MesgJoinAck         *message)
{
  static MesgJoinAck init_value = MESG_JOIN_ACK__INIT;
  *message = init_value;
}
size_t mesg_join_ack__get_packed_size
                     (const MesgJoinAck *message)
{
  assert(message->base.descriptor == &mesg_join_ack__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_join_ack__pack
                     (const MesgJoinAck *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_join_ack__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_join_ack__pack_to_buffer
                     (const MesgJoinAck *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_join_ack__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgJoinAck *
       mesg_join_ack__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgJoinAck *)
     protobuf_c_message_unpack (&mesg_join_ack__descriptor,
                                allocator, len, data);
}
void   mesg_join_ack__free_unpacked
                     (MesgJoinAck *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_join_ack__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_unjoin_req__init
                     (MesgUnjoinReq         *message)
{
  static MesgUnjoinReq init_value = MESG_UNJOIN_REQ__INIT;
  *message = init_value;
}
size_t mesg_unjoin_req__get_packed_size
                     (const MesgUnjoinReq *message)
{
  assert(message->base.descriptor == &mesg_unjoin_req__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_unjoin_req__pack
                     (const MesgUnjoinReq *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_unjoin_req__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_unjoin_req__pack_to_buffer
                     (const MesgUnjoinReq *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_unjoin_req__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgUnjoinReq *
       mesg_unjoin_req__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgUnjoinReq *)
     protobuf_c_message_unpack (&mesg_unjoin_req__descriptor,
                                allocator, len, data);
}
void   mesg_unjoin_req__free_unpacked
                     (MesgUnjoinReq *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_unjoin_req__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_room__init
                     (MesgRoom         *message)
{
  static MesgRoom init_value = MESG_ROOM__INIT;
  *message = init_value;
}
size_t mesg_room__get_packed_size
                     (const MesgRoom *message)
{
  assert(message->base.descriptor == &mesg_room__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_room__pack
                     (const MesgRoom *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_room__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_room__pack_to_buffer
                     (const MesgRoom *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_room__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgRoom *
       mesg_room__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgRoom *)
     protobuf_c_message_unpack (&mesg_room__descriptor,
                                allocator, len, data);
}
void   mesg_room__free_unpacked
                     (MesgRoom *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_room__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_room_ack__init
                     (MesgRoomAck         *message)
{
  static MesgRoomAck init_value = MESG_ROOM_ACK__INIT;
  *message = init_value;
}
size_t mesg_room_ack__get_packed_size
                     (const MesgRoomAck *message)
{
  assert(message->base.descriptor == &mesg_room_ack__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_room_ack__pack
                     (const MesgRoomAck *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_room_ack__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_room_ack__pack_to_buffer
                     (const MesgRoomAck *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_room_ack__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgRoomAck *
       mesg_room_ack__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgRoomAck *)
     protobuf_c_message_unpack (&mesg_room_ack__descriptor,
                                allocator, len, data);
}
void   mesg_room_ack__free_unpacked
                     (MesgRoomAck *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_room_ack__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_group__init
                     (MesgGroup         *message)
{
  static MesgGroup init_value = MESG_GROUP__INIT;
  *message = init_value;
}
size_t mesg_group__get_packed_size
                     (const MesgGroup *message)
{
  assert(message->base.descriptor == &mesg_group__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_group__pack
                     (const MesgGroup *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_group__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_group__pack_to_buffer
                     (const MesgGroup *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_group__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgGroup *
       mesg_group__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgGroup *)
     protobuf_c_message_unpack (&mesg_group__descriptor,
                                allocator, len, data);
}
void   mesg_group__free_unpacked
                     (MesgGroup *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_group__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_group_ack__init
                     (MesgGroupAck         *message)
{
  static MesgGroupAck init_value = MESG_GROUP_ACK__INIT;
  *message = init_value;
}
size_t mesg_group_ack__get_packed_size
                     (const MesgGroupAck *message)
{
  assert(message->base.descriptor == &mesg_group_ack__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_group_ack__pack
                     (const MesgGroupAck *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_group_ack__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_group_ack__pack_to_buffer
                     (const MesgGroupAck *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_group_ack__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgGroupAck *
       mesg_group_ack__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgGroupAck *)
     protobuf_c_message_unpack (&mesg_group_ack__descriptor,
                                allocator, len, data);
}
void   mesg_group_ack__free_unpacked
                     (MesgGroupAck *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_group_ack__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_lsn_rpt__init
                     (MesgLsnRpt         *message)
{
  static MesgLsnRpt init_value = MESG_LSN_RPT__INIT;
  *message = init_value;
}
size_t mesg_lsn_rpt__get_packed_size
                     (const MesgLsnRpt *message)
{
  assert(message->base.descriptor == &mesg_lsn_rpt__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_lsn_rpt__pack
                     (const MesgLsnRpt *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_lsn_rpt__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_lsn_rpt__pack_to_buffer
                     (const MesgLsnRpt *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_lsn_rpt__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgLsnRpt *
       mesg_lsn_rpt__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgLsnRpt *)
     protobuf_c_message_unpack (&mesg_lsn_rpt__descriptor,
                                allocator, len, data);
}
void   mesg_lsn_rpt__free_unpacked
                     (MesgLsnRpt *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_lsn_rpt__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   mesg_kick_req__init
                     (MesgKickReq         *message)
{
  static MesgKickReq init_value = MESG_KICK_REQ__INIT;
  *message = init_value;
}
size_t mesg_kick_req__get_packed_size
                     (const MesgKickReq *message)
{
  assert(message->base.descriptor == &mesg_kick_req__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t mesg_kick_req__pack
                     (const MesgKickReq *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &mesg_kick_req__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t mesg_kick_req__pack_to_buffer
                     (const MesgKickReq *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &mesg_kick_req__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
MesgKickReq *
       mesg_kick_req__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (MesgKickReq *)
     protobuf_c_message_unpack (&mesg_kick_req__descriptor,
                                allocator, len, data);
}
void   mesg_kick_req__free_unpacked
                     (MesgKickReq *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &mesg_kick_req__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
static const ProtobufCFieldDescriptor mesg_online_req__field_descriptors[6] =
{
  {
    "uid",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineReq, uid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "sid",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineReq, sid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "token",
    3,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineReq, token),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "app",
    4,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineReq, app),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "version",
    5,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineReq, version),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "terminal",
    6,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_UINT32,
    offsetof(MesgOnlineReq, has_terminal),
    offsetof(MesgOnlineReq, terminal),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_online_req__field_indices_by_name[] = {
  3,   /* field[3] = app */
  1,   /* field[1] = sid */
  5,   /* field[5] = terminal */
  2,   /* field[2] = token */
  0,   /* field[0] = uid */
  4,   /* field[4] = version */
};
static const ProtobufCIntRange mesg_online_req__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 6 }
};
const ProtobufCMessageDescriptor mesg_online_req__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_online_req",
  "MesgOnlineReq",
  "MesgOnlineReq",
  "",
  sizeof(MesgOnlineReq),
  6,
  mesg_online_req__field_descriptors,
  mesg_online_req__field_indices_by_name,
  1,  mesg_online_req__number_ranges,
  (ProtobufCMessageInit) mesg_online_req__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_online_ack__field_descriptors[7] =
{
  {
    "uid",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineAck, uid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "sid",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineAck, sid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "app",
    3,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineAck, app),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "version",
    4,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineAck, version),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "terminal",
    5,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_UINT32,
    offsetof(MesgOnlineAck, has_terminal),
    offsetof(MesgOnlineAck, terminal),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "code",
    6,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineAck, code),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "errmsg",
    7,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgOnlineAck, errmsg),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_online_ack__field_indices_by_name[] = {
  2,   /* field[2] = app */
  5,   /* field[5] = code */
  6,   /* field[6] = errmsg */
  1,   /* field[1] = sid */
  4,   /* field[4] = terminal */
  0,   /* field[0] = uid */
  3,   /* field[3] = version */
};
static const ProtobufCIntRange mesg_online_ack__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 7 }
};
const ProtobufCMessageDescriptor mesg_online_ack__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_online_ack",
  "MesgOnlineAck",
  "MesgOnlineAck",
  "",
  sizeof(MesgOnlineAck),
  7,
  mesg_online_ack__field_descriptors,
  mesg_online_ack__field_indices_by_name,
  1,  mesg_online_ack__number_ranges,
  (ProtobufCMessageInit) mesg_online_ack__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_join_req__field_descriptors[3] =
{
  {
    "uid",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgJoinReq, uid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "rid",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgJoinReq, rid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "token",
    3,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgJoinReq, token),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_join_req__field_indices_by_name[] = {
  1,   /* field[1] = rid */
  2,   /* field[2] = token */
  0,   /* field[0] = uid */
};
static const ProtobufCIntRange mesg_join_req__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 3 }
};
const ProtobufCMessageDescriptor mesg_join_req__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_join_req",
  "MesgJoinReq",
  "MesgJoinReq",
  "",
  sizeof(MesgJoinReq),
  3,
  mesg_join_req__field_descriptors,
  mesg_join_req__field_indices_by_name,
  1,  mesg_join_req__number_ranges,
  (ProtobufCMessageInit) mesg_join_req__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_join_ack__field_descriptors[5] =
{
  {
    "uid",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgJoinAck, uid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "rid",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgJoinAck, rid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "gid",
    3,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgJoinAck, gid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "code",
    4,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgJoinAck, code),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "errmsg",
    5,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgJoinAck, errmsg),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_join_ack__field_indices_by_name[] = {
  3,   /* field[3] = code */
  4,   /* field[4] = errmsg */
  2,   /* field[2] = gid */
  1,   /* field[1] = rid */
  0,   /* field[0] = uid */
};
static const ProtobufCIntRange mesg_join_ack__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 5 }
};
const ProtobufCMessageDescriptor mesg_join_ack__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_join_ack",
  "MesgJoinAck",
  "MesgJoinAck",
  "",
  sizeof(MesgJoinAck),
  5,
  mesg_join_ack__field_descriptors,
  mesg_join_ack__field_indices_by_name,
  1,  mesg_join_ack__number_ranges,
  (ProtobufCMessageInit) mesg_join_ack__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_unjoin_req__field_descriptors[2] =
{
  {
    "uid",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgUnjoinReq, uid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "rid",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgUnjoinReq, rid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_unjoin_req__field_indices_by_name[] = {
  1,   /* field[1] = rid */
  0,   /* field[0] = uid */
};
static const ProtobufCIntRange mesg_unjoin_req__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 2 }
};
const ProtobufCMessageDescriptor mesg_unjoin_req__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_unjoin_req",
  "MesgUnjoinReq",
  "MesgUnjoinReq",
  "",
  sizeof(MesgUnjoinReq),
  2,
  mesg_unjoin_req__field_descriptors,
  mesg_unjoin_req__field_indices_by_name,
  1,  mesg_unjoin_req__number_ranges,
  (ProtobufCMessageInit) mesg_unjoin_req__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_room__field_descriptors[7] =
{
  {
    "uid",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgRoom, uid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "rid",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgRoom, rid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "gid",
    3,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgRoom, gid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "level",
    4,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgRoom, level),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "time",
    5,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgRoom, time),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "text",
    6,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgRoom, text),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "data",
    7,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_BYTES,
    offsetof(MesgRoom, has_data),
    offsetof(MesgRoom, data),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_room__field_indices_by_name[] = {
  6,   /* field[6] = data */
  2,   /* field[2] = gid */
  3,   /* field[3] = level */
  1,   /* field[1] = rid */
  5,   /* field[5] = text */
  4,   /* field[4] = time */
  0,   /* field[0] = uid */
};
static const ProtobufCIntRange mesg_room__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 7 }
};
const ProtobufCMessageDescriptor mesg_room__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_room",
  "MesgRoom",
  "MesgRoom",
  "",
  sizeof(MesgRoom),
  7,
  mesg_room__field_descriptors,
  mesg_room__field_indices_by_name,
  1,  mesg_room__number_ranges,
  (ProtobufCMessageInit) mesg_room__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_room_ack__field_descriptors[2] =
{
  {
    "code",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgRoomAck, code),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "errmsg",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgRoomAck, errmsg),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_room_ack__field_indices_by_name[] = {
  0,   /* field[0] = code */
  1,   /* field[1] = errmsg */
};
static const ProtobufCIntRange mesg_room_ack__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 2 }
};
const ProtobufCMessageDescriptor mesg_room_ack__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_room_ack",
  "MesgRoomAck",
  "MesgRoomAck",
  "",
  sizeof(MesgRoomAck),
  2,
  mesg_room_ack__field_descriptors,
  mesg_room_ack__field_indices_by_name,
  1,  mesg_room_ack__number_ranges,
  (ProtobufCMessageInit) mesg_room_ack__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_group__field_descriptors[6] =
{
  {
    "uid",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgGroup, uid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "gid",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgGroup, gid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "level",
    3,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgGroup, level),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "time",
    4,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT64,
    0,   /* quantifier_offset */
    offsetof(MesgGroup, time),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "text",
    5,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgGroup, text),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "data",
    6,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_BYTES,
    offsetof(MesgGroup, has_data),
    offsetof(MesgGroup, data),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_group__field_indices_by_name[] = {
  5,   /* field[5] = data */
  1,   /* field[1] = gid */
  2,   /* field[2] = level */
  4,   /* field[4] = text */
  3,   /* field[3] = time */
  0,   /* field[0] = uid */
};
static const ProtobufCIntRange mesg_group__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 6 }
};
const ProtobufCMessageDescriptor mesg_group__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_group",
  "MesgGroup",
  "MesgGroup",
  "",
  sizeof(MesgGroup),
  6,
  mesg_group__field_descriptors,
  mesg_group__field_indices_by_name,
  1,  mesg_group__number_ranges,
  (ProtobufCMessageInit) mesg_group__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_group_ack__field_descriptors[2] =
{
  {
    "code",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgGroupAck, code),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "errmsg",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgGroupAck, errmsg),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_group_ack__field_indices_by_name[] = {
  0,   /* field[0] = code */
  1,   /* field[1] = errmsg */
};
static const ProtobufCIntRange mesg_group_ack__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 2 }
};
const ProtobufCMessageDescriptor mesg_group_ack__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_group_ack",
  "MesgGroupAck",
  "MesgGroupAck",
  "",
  sizeof(MesgGroupAck),
  2,
  mesg_group_ack__field_descriptors,
  mesg_group_ack__field_indices_by_name,
  1,  mesg_group_ack__number_ranges,
  (ProtobufCMessageInit) mesg_group_ack__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_lsn_rpt__field_descriptors[5] =
{
  {
    "nid",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgLsnRpt, nid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "nation",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgLsnRpt, nation),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "name",
    3,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgLsnRpt, name),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "ipaddr",
    4,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgLsnRpt, ipaddr),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "port",
    5,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgLsnRpt, port),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_lsn_rpt__field_indices_by_name[] = {
  3,   /* field[3] = ipaddr */
  2,   /* field[2] = name */
  1,   /* field[1] = nation */
  0,   /* field[0] = nid */
  4,   /* field[4] = port */
};
static const ProtobufCIntRange mesg_lsn_rpt__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 5 }
};
const ProtobufCMessageDescriptor mesg_lsn_rpt__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_lsn_rpt",
  "MesgLsnRpt",
  "MesgLsnRpt",
  "",
  sizeof(MesgLsnRpt),
  5,
  mesg_lsn_rpt__field_descriptors,
  mesg_lsn_rpt__field_indices_by_name,
  1,  mesg_lsn_rpt__number_ranges,
  (ProtobufCMessageInit) mesg_lsn_rpt__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor mesg_kick_req__field_descriptors[2] =
{
  {
    "code",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_UINT32,
    0,   /* quantifier_offset */
    offsetof(MesgKickReq, code),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "errmsg",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(MesgKickReq, errmsg),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned mesg_kick_req__field_indices_by_name[] = {
  0,   /* field[0] = code */
  1,   /* field[1] = errmsg */
};
static const ProtobufCIntRange mesg_kick_req__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 2 }
};
const ProtobufCMessageDescriptor mesg_kick_req__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "mesg_kick_req",
  "MesgKickReq",
  "MesgKickReq",
  "",
  sizeof(MesgKickReq),
  2,
  mesg_kick_req__field_descriptors,
  mesg_kick_req__field_indices_by_name,
  1,  mesg_kick_req__number_ranges,
  (ProtobufCMessageInit) mesg_kick_req__init,
  NULL,NULL,NULL    /* reserved[123] */
};
