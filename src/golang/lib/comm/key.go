...
const (
	...
	//|**宏**|**键值**|**类型**|**描述**|**备注**|
	IM_KEY_LSND_NETWORK_ZSET   = "im:lsnd:network:zset"                           //| ZSET | 帧听层"网络类型"集合 | 成员:"网络类型" 分值:TTL |
	IM_KEY_LSND_NATION_ZSET    = "im:lsnd:network:%d:nation:zset"                 //| ZSET | 某"网络类型"的帧听层"地区/国家"集合 | 成员:"国家/地区" 分值:TTL |
	IM_KEY_LSND_OP_ZSET        = "im:lsnd:network:%d:nation:%s:op:zset"           //| ZSET | 帧听层"地区/国家"对应的运营商集合 | 成员:运营商名称 分值:TTL |
	IM_KEY_LSND_IP_ZSET        = "im:lsnd:network:%d:nation:%s:op:%s:zset"        //| ZSET | 帧听层"地区/国家"-运营商对应的IP集合 | 成员:IP 分值:TTL |
	IM_KEY_LSND_OP_TO_NID_ZSET = "im:lsnd:network:%d:nation:%s:op:%s:to:nid:zset" //| ZSET | 运营商帧听层NID集合 | 成员:NID 分值:TTL |
