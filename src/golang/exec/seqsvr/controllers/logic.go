 ...
 ******************************************************************************/
func (ctx *SeqSvrCntx) timer_clean() {
	for {
		ctm := time.Now().Unix()
 ...
 ******************************************************************************/
func (ctx *SeqSvrCntx) timer_update() {
	for {
 ...
 **作    者: # Qifeng.zou # 2016.11.04 12:08:43 #
 ******************************************************************************/
func (ctx *SeqSvrCntx) clean_sid_zset(ctm int64) {
	rds := ctx.redis.Get()
	defer rds.Close()
 ...
 ******************************************************************************/
func (ctx *SeqSvrCntx) clean_sid_by_rid(rid uint64) {
	rds := ctx.redis.Get()
	defer rds.Close()
 ...
 **作    者: # Qifeng.zou # 2016.11.04 17:16:38 #
 ******************************************************************************/
func (ctx *SeqSvrCntx) clean_uid_by_rid(rid uint64) {
	rds := ctx.redis.Get()
	defer rds.Close()
 ...
 **作    者: # Qifeng.zou # 2016.11.04 12:08:43 #
 ******************************************************************************/
func (ctx *SeqSvrCntx) clean_by_rid(rid uint64) {
	pl := ctx.redis.Get()
	defer func() {
		pl.Do("")
		pl.Close()
 ...
 **作    者: # Qifeng.zou # 2016.11.04 12:08:43 #
 ******************************************************************************/
func (ctx *SeqSvrCntx) clean_rid_zset(ctm int64) {
	rds := ctx.redis.Get()
	defer rds.Close()
 ...
 ******************************************************************************/
func (ctx *SeqSvrCntx) clean_by_uid(uid uint64) {
	pl := ctx.redis.Get()
	defer func() {
		pl.Do("")
		pl.Close()
 ...
 ******************************************************************************/
func (ctx *SeqSvrCntx) clean_uid_zset(ctm int64) {
	rds := ctx.redis.Get()
	defer rds.Close()
 ...
 ******************************************************************************/
func (ctx *SeqSvrCntx) update_prec_statis() {
	rds := ctx.redis.Get()
	defer rds.Close()
 ...
 **作    者: # Qifeng.zou # 2016.11.04 17:49:10 #
 ******************************************************************************/
func (ctx *SeqSvrCntx) clean_prec_statis() {
	rds := ctx.redis.Get()
	defer rds.Close()
